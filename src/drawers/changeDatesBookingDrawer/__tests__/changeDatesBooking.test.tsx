...
describe("<ChangeDatesBookingDrawer />", () => {
  ...
  test("should call setConfirmedBookings on submit", () => {
    ...
    const checkOutDate = moment(defaultProps.bookingData.checkOut)
      .add(1, "days")
      .format("YYYY-MM-DD");
  ...
  test("should render an error message whenever there are dates overlapping on submit", () => {
    const wrapper = render(
      setup({
        ...
        confirmedBookings: [{ ...testBookingData, id: "2" }],
      })
    );
  ...
  test("should update the context with the new booking data on submit", () => {
    const setConfirmedBookingsMock = jest.fn();
    const wrapper = render(
      setup({
        ...defaultConfirmedBookingsContextValue,
        setConfirmedBookings: setConfirmedBookingsMock,
      })
    );
    ...
    expect(setConfirmedBookingsMock).toHaveBeenCalledWith([
      {
        ...defaultProps.bookingData,
